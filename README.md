...
| Verification | 188 Rust tests (unit, property, differential, scenario, end to end) and 77 Python cases, run in CI on Linux and Windows |
...
A limit order book holds resting orders at discrete price levels. An order that trades immediately consumes them from the best price outward. If the ask side offers sizes $`s_1, s_2, \dots`$ at prices $`p_1 < p_2 < \dots`$, a market buy for $`Q`$ units pays an average of

```math
\bar{P}(Q) \;=\; \frac{1}{Q} \sum_{i} p_i \cdot \min\!\Big(s_i,\; \max\big(0,\; Q - \textstyle\sum_{j<i} s_j\big)\Big)
```

Every unit beyond the depth at the best price costs more than the one before it. With 20,000 units at 100.50, 30,000 at 101.00, 40,000 at 101.50 and 50,000 at 102.00, a single order for 100,000 units averages **101.20**, about 70 basis points above the best ask. One sixth of the same order, 16,667 units, fits entirely at the best ask.
...
| POV | volume actually observed | caps the order's share of the market, accepts an uncertain finish time |
| Implementation shortfall | an explicit cost and risk model | minimises expected cost plus a penalty on its variance |

### How execution quality is measured

Execution cost is reported as **implementation shortfall** against the arrival price, the mid price at the moment the order was accepted. For an average fill price $`\bar{P}`$ and arrival mid $`M_0`$,

```math
\text{shortfall}_{\text{bps}} \;=\; d \cdot \frac{\bar{P} - M_0}{M_0} \cdot 10^4, \qquad d = \begin{cases} +1 & \text{buy} \\ -1 & \text{sell} \end{cases}
```

A positive value means the execution was worse than arrival: paying more on a buy, or receiving less on a sell. The engine reports this for every parent order.
...
| [`api`](crates/api) | Protobuf schema, generated gRPC code, the tonic service and the command types shared with the engine | `orderbook` |
| [`config`](crates/config) | TOML settings with unknown keys rejected and every value range-checked | none |
...
sequenceDiagram
    ...
    loop every engine tick (50 ms by default)
        E->>V: one IOC child for the quantity now due
        V-->>E: fills
        E->>E: update order state and position
...
1. **A deterministic core with the clock at the edge.** `EngineCore` never reads a clock. Every method that changes state takes the current time as an argument, and only the async wrapper in [`event_loop.rs`](crates/engine/src/event_loop.rs) reads the wall clock. The same commands with the same timestamps therefore produce the same child orders, fills and P&L on a given build and platform, and every engine scenario can be tested exactly.
2. **Integers for prices and money.** Prices are `i64` tick counts, quantities are `u64`, positions are `i64`, and cash, cost basis and P&L are `i128`. Floating point appears only at the boundaries: user input, display, and the Almgren-Chriss model.
...
5. **Layers that test in isolation.** Algorithms test without a clock, the engine tests without a network, and the API tests without the GUI.
...
A `Price` is an `i64` count of ticks, with `TICK_SCALE = 100,000` ticks per price unit (five decimal places). Comparing prices and looking up a level are exact integer operations.
...
**Exactness bound.** Take a price with at most five decimals, written as its nearest `f64` (as a TOML or Python literal gives), with true tick count $`m`$. Two rounding errors separate the computed product $`\hat{x}`$ from $`m`$: one from parsing the decimal, one from the multiplication.

```math
|\hat{x} - m| \;\le\; 10^5 \cdot \tfrac{1}{2}\,\mathrm{ulp}(\hat{p}) \;+\; \tfrac{1}{2}\,\mathrm{ulp}(\hat{x}) \;<\; \tfrac{1}{2} \qquad \text{whenever } |m| < 2^{51}
```

So conversion is exact for tick counts below $`2^{51}`$, prices below about $`2.25 \times 10^{10}`$. Above that the two errors can reach half a tick. Random sampling confirms both sides: no errors in $`[2^{50}, 2^{51})`$, and about 8% of prices off by one tick in $`[2^{51}, 2^{52})`$. `Price::try_from_f64` rejects NaN, infinities and values outside the `i64` range for untrusted input.
...
| Cash, cost basis, P&L, notional | `i128` (`TickValue`) | ticks × units |
| Timestamp | `u64` | nanoseconds since the Unix epoch |

`i128` holds the product of any `u64` quantity and any `i64` price, so money arithmetic cannot silently overflow. Where a sum could in principle exceed it, the code uses checked or saturating arithmetic. The mid price is $`\lfloor (\text{bid} + \text{ask}) / 2 \rfloor`$ ticks.
...
| FOK | the order trades its full quantity immediately, or nothing trades and the book is unchanged |
...
`check_invariants()` walks the whole book in expected $`O(n)`$ time and verifies that:
...
The unit tests call it after every order they submit, and the differential test calls it after every operation.

### Complexity

With $`L`$ price levels on a side and $`k`$ orders at one level (index operations are expected and amortised because the index is a hash map):
...
- at most 1,000,000 slices, and every slice at least 1 ns long.
...
The chart shows 100 units over one hour. TWAP gives 17, 16, 17, 17, 16, 17. VWAP, with profile weights 4, 2, 1, 1, 2, 4, gives 29, 14, 7, 7, 14, 29. POV at 10% of prints of 300, 150, 80, 90, 160 and 320 gives 30, 15, 8, 9, 16, 22, where the last child is capped by the order size. Implementation shortfall with $`\lambda = 0.001`$ gives 41, 25, 14, 9, 6, 5.

### 5.2 TWAP: time-weighted average price

TWAP trades an equal share of the order in each time slice. The subtle part is rounding. Giving the remainder to the first slices front-loads the order, which is the opposite of what TWAP is for. Instead, the **cumulative** target is rounded, and child quantities are its differences:
...
- **Reproducible.** The same seed gives the same schedule on every platform. SplitMix64's output is fixed by its definition, and a test pins its first output, `0xE220A8397B1DCDAF`.
...
SplitMix64 is not a cryptographic generator. The randomization defeats a fixed-clock observer, but anyone who learns the seed can reproduce every release time.
...
**Intuition.** Suppose each slice $`k`$ trades at an average price $`p_k`$ with market volume $`v_k`$, out of a total $`V`$. An order that trades the same share of each slice's volume, $`q_k = X \, v_k / V`$, achieves exactly the market's VWAP:

```math
\bar{P}_{\text{order}} \;=\; \frac{\sum_k q_k\,p_k}{X} \;=\; \frac{\sum_k v_k\,p_k}{V} \;=\; \text{VWAP}_{\text{market}}
```

Future volume is unknown, so the schedule follows a **forecast** profile $`w_0, \dots, w_{N-1}`$ estimated from history. Tracking error against the realised VWAP comes from the forecast being wrong.

**Schedule.** With cumulative shares $`S_k = \sum_{j \le k} w_j / \sum_j w_j`$, the cumulative target after slice $`k`$ is $`\operatorname{round}(X\,S_k)`$. It is clamped to be non-decreasing, and the last slice with volume completes $`X`$ exactly. The curve stops at that slice instead of being forced to 1 afterwards. Above $`2^{53}`$ units a rounded `f64` target cannot land exactly on the total, and forcing the curve would hand the leftover to a trailing zero-volume slice. Truncating it guarantees a slice with no forecast volume never receives a child. Weights may be on any scale; the code divides by the largest weight so the running sum stays finite even for weights near `f64::MAX`.
...
```math
\operatorname{slice}(o) \;=\; \Big\lfloor \frac{(o+1)\,N - 1}{D} \Big\rfloor \quad\Longleftrightarrow\quad \Big\lfloor \frac{kD}{N} \Big\rfloor \;\le\; o \;<\; \Big\lfloor \frac{(k+1)D}{N} \Big\rfloor
```

These are exactly the boundaries the schedulers use, so a bar's volume informs the slice it actually falls in. The simpler $`\lfloor oN/D \rfloor`$ can place a bar one slice early: for a 10 ns window in 3 slices, bars at offsets 3 and 6 would land in slices 0 and 1 instead of 1 and 2. Each slice's fraction is its pooled volume over the pooled window volume, so busier days weigh more. Only bars from before the day being traded should be used, since that day's own volume would be look-ahead.
...
```math
C_j \;=\; \min\!\Big(X,\; \Big\lfloor \frac{\rho \cdot V_j}{10\,000} \Big\rfloor\Big), \qquad \text{child}_j \;=\; C_j - C_{j-1}
```

The arithmetic is exact in `u128`, so $`10\,000 \cdot C_j \le \rho \cdot V_j`$ holds for every prefix, exactly and not approximately. Observations that share a timestamp form one decision point and one child. If the volume never arrives, the untraded quantity is reported as `shortfall_qty` rather than dropped.
...
At 10% participation over the same volume stream, a 5,000-unit order completes at the print 38.5 minutes into the window and stops. A 12,000-unit order meets a window volume of 90,767, so it schedules $`\lfloor 9076.7 \rfloor = 9{,}076`$ units and reports a shortfall of 2,924.
...
Implementation shortfall is the only algorithm here that is an **optimisation** rather than a rule. It follows the discrete model of Almgren and Chriss (2000).
...
The shortfall of a trajectory is a random variable with
...
- **Permanent impact cost** $`\tfrac{1}{2}\gamma X^2`$ is the same for every trajectory, so it cannot be optimised away.
- **Temporary impact cost** $`\sum n_k^2`$ is convex, so for a fixed total it is smallest when trades are equal and grows sharply when trading is concentrated.
- **Risk** $`V`$ is the variance of what is still held, so it is smallest when the order is finished quickly.
...
For a risk aversion $`\lambda \ge 0`$, the engine minimises $`U = E + \lambda V`$. Because $`\tilde{\eta} > 0`$, $`U`$ is a strictly convex quadratic in the interior holdings, so its unique minimum is where every partial derivative vanishes. Setting $`\partial U / \partial x_k = 0`$ gives a linear second-order difference equation:
...
```math
x_j \;=\; X\,\frac{\sinh\!\big(\kappa\,(T - t_j)\big)}{\sinh(\kappa T)}, \qquad \frac{2}{\tau^2}\big(\cosh(\kappa\tau) - 1\big) \;=\; \tilde{\kappa}^2
```

**Reading the result.**

- $`\kappa`$ is the **urgency**, with units of 1/time, and $`1/\kappa`$ is the time scale on which holdings decay.
- **Risk-neutral** ($`\lambda = 0`$): $`\kappa = 0`$, the sinh ratio becomes the straight line $`(T - t_j)/T`$, and the schedule is TWAP. The code uses TWAP's integer rounding in this case, and a test checks the children are identical to TWAP's for every slice count from 1 to 39 against 203 order sizes.
...
- **Fine slices** ($`\tau \to 0`$): $`\kappa \to \tilde{\kappa} = \sqrt{\lambda\sigma^2/\tilde{\eta}}`$, the square root of risk per unit time over impact cost.

![Optimal holdings and how the urgency scales](docs/images/almgren_chriss_trajectories.svg)

#### Worked numbers

The table uses 100,000 units over one hour in 60 one-minute slices, with $`\sigma = 0.02`$, $`\eta = 0.5`$, $`\gamma = 10^{-6}`$ (so $`\tilde{\eta} = 0.49997`$). Every value comes from `almgren_chriss_schedule`:
...
- **Continuous-time limit.** For $`\lambda \le 0.001`$ the discrete $`\kappa`$ is within 0.02% of $`\sqrt{\lambda\sigma^2/\tilde{\eta}}`$. As urgency grows, the one-minute slice starts to bind: at $`\lambda = 0.1`$, $`\kappa`$ is 1.2% below the limit.
...
- **Rounding.** At $`\lambda = 0.1`$ the order is essentially done in 22 minutes, and the remaining slices round to zero and are omitted.
...
The closed form is simple on paper, but three places need care in floating point. Each has a test at the extreme that used to fail:

- **Tiny urgency.** $`\kappa\tau = \operatorname{acosh}(1 + y)`$ with $`y = \tilde{\kappa}^2\tau^2/2`$. For tiny $`y`$, `1 + y` rounds to exactly 1 and `acosh` returns 0, which would silently turn the schedule into TWAP. The code evaluates $`\kappa\tau = \operatorname{ln1p}\big(y + \sqrt{y(y+2)}\big)`$, which keeps the $`\sqrt{2y}`$ behaviour. For large $`y`$ it factors $`y`$ out of the square root so the product cannot overflow.
- **Huge urgency.** At high $`\kappa T`$, `sinh` overflows and the ratio becomes `inf/inf`. The code evaluates it as $`e^{-\kappa t}\,\frac{\operatorname{expm1}(-2\kappa(T-t))}{\operatorname{expm1}(-2\kappa T)}`$, which stays finite for $`\lambda`$ as large as $`10^{200}`$.
- **Degenerate inputs.** $`\tilde{\eta} \le 0`$ is rejected, because the model divides by it. The variance is computed as $`\tau\sum(\sigma x_k)^2`$ rather than $`\sigma^2\tau\sum x_k^2`$, so an overflowing $`\sigma^2`$ times an empty trajectory gives 0 instead of `NaN`.
...
`EngineCore` holds the simulated venue, every parent order, positions, risk limits and the halt state. It changes only through `submit`, `cancel`, `tick` and `halt`, and it is read through `order_status` and `positions`. Every one of those calls receives the time as an argument.
...
Four behaviours follow from this loop:
...
Pre-trade checks run on every new parent order. Let $`Q`$ be the order quantity, $`L`$ its limit price (if any), $`M`$ the mid, $`x`$ the current position, and $`W^{\text{buy}}`$, $`W^{\text{sell}}`$ the unfilled quantity of working orders in the symbol:

| Check | Condition for acceptance |
| --- | --- |
| Order size | $`Q \le Q_{\max}`$ |
| Price collar (limit orders) | $`\lvert L - M\rvert \cdot 10^4 \le c_{\text{bps}} \cdot M`$ |
| Order notional | $`Q \cdot \max(L, M) \le N_{\max}`$, using $`M`$ for a market order |
...
- **Closing $`c`$ units:** realized P&L gains the closing value minus the removed cost, and the cost basis loses the removed cost. Their difference therefore moves by exactly the closing value, which is the cash that changed hands.

Unrealized P&L is $`x \cdot \text{mark} - \text{cost basis}`$, so total P&L is $`\text{net cash} + x \cdot \text{mark}`$: plain mark-to-market, with no accumulated rounding. A property test applies random fill sequences of up to 199 fills and checks the identity after every fill. When $`c`$ of $`o`$ open units close, the cost removed is $`\operatorname{trunc}(B\,c/o)`$ for cost basis $`B`$. The code splits $`B`$ into its whole and remainder parts relative to $`o`$ and scales each separately. This gives the same integer without forming the product $`B\,c`$, which could overflow even when the result fits.
...
- **One owner.** The engine task owns `EngineCore`. Handlers send a command carrying a fresh oneshot channel and await the reply, so there is no locking anywhere in the engine.
...
- **Shutdown order.** A graceful gRPC shutdown waits for open streams to finish, and a fill stream only finishes when the engine stops. `engine::serve` therefore stops the engine *as part of* the shutdown signal, before the server drains. An end-to-end test holds a `StreamFills` stream open through shutdown and checks the server exits.
...
sequenceDiagram
    ...
    Srv->>Eng: wait for the task to exit
```
...
Schema: [`proto/execution.proto`](proto/execution.proto). Prices are integer ticks, quantities integer units, timestamps Unix epoch nanoseconds, and P&L ticks × units (clamped to `int64` on the wire). Algorithm parameters use a `oneof`, and optional values use `optional` fields rather than magic numbers.
...
| `GetPositions` | per symbol: quantity, average price, realized and unrealized P&L, mark price; plus whether trading has halted and why |
...
The API separates a **business answer** from a **failed call**, so a client never retries something it should show to a user:
...
An egui application that acts as a thin client. Every order, position and fill it shows comes from an engine reply. It has four pages:

| Page | Contents |
| --- | --- |
| Orders | order entry for TWAP, VWAP and implementation shortfall; a table of submitted orders with their shortfall; a detail panel with the limit, window and child orders |
...
| Connection | engine address (default `127.0.0.1:50051`), connect and disconnect |

A status bar above every page shows the connection state and, in red, whether trading has halted and why. The client does not connect automatically; connect from the Connection page. It is built to stay correct when the engine is slow, restarted or unreachable:

- **Sessions.** Each connection is a session. Connecting or disconnecting clears what the previous session showed, and a session generation number discards replies and fills that arrive late from an old connection.
- **Deadlines.** Every request has a 5-second deadline, and connecting times out after 3 seconds.
- **Backpressure.** At most one request of each kind is in flight. Polling stops for an order the engine no longer knows, and the fill stream resubscribes with backoff.
...
The only local values are the three quick-fill symbol buttons (the API has no call to list symbols), the form defaults, and log times.
...
A PyO3 extension built with maturin. The `algo_exec_py` package ships with the compiled module inside it as `algo_exec_py._native` and re-exports its functions. Supported CPython versions are 3.9 to 3.13 (`requires-python = ">=3.9,<3.14"`, matching PyO3 0.22).
...
| Value the scheduler rejects | `ValueError`, carrying the Rust error message |

**Backtest harness.** `algo_exec_py.backtest.BacktestEngine` replays a TWAP or randomized TWAP schedule against a recorded price path. Each child fills in full at the price linearly interpolated at its release time, clamped at the ends of the path. `calculate_metrics()` reports total quantity, notional, VWAP, the arrival price and shortfall in basis points. It is a no-impact, no-latency baseline that isolates how a schedule's timing interacts with price moves. For example, buying 1,000 units in 10 slices while the price rises from 100 by 1.0 per second gives a VWAP of 104.5 and a shortfall of +450 bps; selling gives -450 bps.
...
| `algo-core` | 59 | 5 | 0 |
| `engine` | 21 | 1 | 23 (21 deterministic scenarios, 2 end to end over gRPC) |
| `api` | 6 | 1 | 0 |
| `config` | 4 | 0 | 0 |
...
Python: 38 test functions, 77 cases once parametrised.
...
- **Closed forms and known answers**, where the expected value comes from outside the implementation:
  - SplitMix64's published first output;
  ...
  - $`\lambda = 0`$ reproducing TWAP exactly;
...
- **Corruption tests.** Private book state is corrupted in each way `check_invariants` should detect: a crossed book, a stale cached total, a missing or stale index entry, a duplicate id at two levels, a duplicate id that only the id check can catch, an empty level, a non-positive price, and a resting IOC order. Each must be reported.
- **End to end.** Over a real loopback socket, one test submits an order, streams its fills, and reads status and positions. It then checks that a business rejection arrives in the response while a malformed request gets `INVALID_ARGUMENT` and an unknown id gets `NOT_FOUND`. A second test holds a fill stream open through shutdown.
...
- **Rust**, on Ubuntu and Windows: `cargo fmt --check`, `cargo clippy --workspace --all-targets --locked -- -D warnings`, then the full test suite in debug (integer overflow checks on) and in release.
- **Python**, on CPython 3.9 and 3.13: build and install the package, then run pytest.

---

## 11. Performance

Criterion benchmarks of the order book on the `bench` profile (release with LTO, one codegen unit), single-threaded on one laptop. These measure book operations inside one process; they are not end-to-end latency figures for the engine or the API.
...
Each per-operation benchmark restores the book to the same shape after every iteration, so the cost does not drift as it runs. The mixed workload is generated against a shadow copy of the book, so cancels name orders that are really resting and every IOC is priced through the live touch and trades. The add and cancel choice caps the resting count, and the benchmark asserts the book ends within its band of the starting size, so the figure describes a steady state rather than a growing book.
...
- **The market model is static.** The reference price never moves and the market maker is the only liquidity. Fills consume depth, which the next tick restores. Execution cost measured in the engine is the scheduler walking a static ladder, not market impact in a real market.
...
- **POV is not exposed through the engine API**, because the venue publishes no volume feed.
...
Requirements: a recent stable Rust toolchain. `protoc` is vendored, so no system protobuf installation is needed. On Linux the GUI needs the usual X11 and xkbcommon development libraries; CI installs `libxcb-render0-dev libxcb-shape0-dev libxcb-xfixes0-dev libxkbcommon-dev libssl-dev`.
...
Settings live in [`config/default.toml`](config/default.toml). Unknown keys are rejected, every value is range-checked with the offending field named, and money is written in price units and converted to ticks.
...
Every chart except the benchmark chart is computed from the library through the Python bindings:
...
├── docs/
│   ├── generate_charts.py        builds the README charts from the library
...
- Steele, G. L., Lea, D. and Flood, C. H. (2014). "Fast splittable pseudorandom number generators." *OOPSLA 2014*. The SplitMix64 generator used for randomized release times.
